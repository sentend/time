...
export default async (
	...
	const workspaces = userWorkspaces.map(userWorkspace => userWorkspace.workspace);

	let currentWorkspace: Workspace | undefined = undefined;
	if (resolveWorkspaceId !== undefined) {
		// Find workspace
		currentWorkspace =
			workspaces.find(workspace => workspace.id === resolveWorkspaceId) ?? workspaces[0];
	} else {
	...
	return {
		...
		workspaces: workspaces.map(toIWorkspace),
